...
(() => {
  ...
  renderer.code = (token) => {
    ...
    const copyIconLight = window.__sparkCopyIconLight;
    const copyIconDark = window.__sparkCopyIconDark;
    const iconMarkup = copyIconLight && copyIconDark
      ? [
          '<picture class="code-block__copy-icon" aria-hidden="true">',
          `<source media="(prefers-color-scheme: dark)" srcset="${copyIconDark}">`,
          `<img src="${copyIconLight}" alt="">`,
          '</picture>'
        ].join('')
      : [
