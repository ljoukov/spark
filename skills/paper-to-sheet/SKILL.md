...
- In compact grading-report mode, do not rewrite source questions into summaries such as `Figure 1 asks...` or `Table 1 gives...`. Copy the exact source question wording for every answered item, omitting only irrelevant administration chrome. Do not use `Use Figure N in the linked original PDF.` as the default or first-pass representation for source-paper visuals. Embed every named source table as Markdown and every named source PDF figure/diagram/graph/chart/option block as a validated crop near the source-faithful prompt when source pixels/table text are available; for uploaded photos/scans, use one JPEG source image with a localized `#spark-bbox` viewport. Preserve source column arithmetic, number grids, stacked calculations, flow/box layouts, and circled/lettered labels with visible Markdown, simple horizontal/vertical LaTeX display layout, or a source-backed image; never flatten them into a prose cue. Use a linked-source-PDF instruction only when the source visual is genuinely unavailable in the working source or a real bounded render/crop/table attempt failed, and record that failure in the artifact plan plus the source-fidelity audit. Artifact validation rejects ordinary named figures/tables that are present in the source but only referenced through the linked original/source PDF.
...
- Validate only final linked PDF crop assets. Scratch inventory paths such as `grader/output/pdf-images/...` are not worksheet assets and must not be linked directly. If an extracted PDF image object is already the right final visual, call `crop_image` with `fullImage: true` and the exact final `outputPath` from `sheet-plan.md` under `grader/output/assets/...`, then link and validate that guarded asset. For PDF crops, the image link target should be the full rendered source-page JPEG under `grader/output/source-pages/...` / `sheet/output/source-pages/...`. For uploaded photos/scans, use a source JPEG with `#spark-bbox=left,top,right,bottom` and record the two-pass `view_image` check in the plan/source-fidelity notes instead of `crop-validation.md`. Do not create `-raw`, `-candidate`, `-draft`, or temp files under `grader/output/assets`; first-pass assets should already be the paths linked in `sheet.json`. If `pad_image` adds a border to a crop that already passed validation, do not call `validate_crop_with_fresh_agent` again just because of padding; record the final padded path in `crop-validation.md` and note the passed source validation. If the original validation failed only because all required content touched an edge, pad once and validate that padded asset only.
...
- Convert textual/numeric source tables into Markdown tables whenever possible. If the source prompt says "Complete the table", the visible worksheet prompt must contain that table or, when the table is genuinely visual/layout-heavy, a source-backed image of it.
- Preserve source-authored stacked arithmetic and number grids as renderable display LaTeX or a source-backed image. Use display LaTeX only for simple layouts made from horizontal/vertical lines, grids, boxes, text, and numbers, such as arithmetic arrays or a small number grid. In JSON strings, LaTeX row breaks must contain two backslash characters in the final Markdown, which means escaping them as `\\\\` inside the JSON string. Do not use fenced code blocks for arithmetic layouts; the renderer adds code UI chrome that is not part of the source paper.
- Treat mathematical number grids, arranged arithmetic, matrices, and display-only sign/number layouts as math layouts rather than data tables. Preserve them with display LaTeX (`array`, matrix, or aligned environments with borders where appropriate) only when every visible line is horizontal or vertical. For angles, geometry diagrams, 3D solids, graphs, charts, maps, networks, photos, apparatus, and any complex or ambiguous visual, use a source-backed image instead.
...
- Do not use SVG in worksheet prompts or linked worksheet images. If the visual is too complex for simple horizontal/vertical LaTeX, use real source pixels.
- For PDF source material, display a real figure/table/options crop under `grader/output/assets/...` or `sheet/output/assets/...`, but make the click target the full rendered PDF page as a JPEG no larger than 1500px on either side, for example `[![Figure 1](grader/output/assets/q1-figure-1.jpg)](grader/output/source-pages/page-0003.jpg)`.
- For uploaded photos/scans, prefer one full-photo JPEG no larger than 1500px on either side and show localized regions with a `#spark-bbox=left,top,right,bottom` fragment measured on the EXIF-oriented JPEG resized to fit within 1500px on each side, for example `[![Figure 1](grader/uploads/photo.jpg#spark-bbox=120,240,960,700)](grader/uploads/photo.jpg)`. This lets several figures reuse the same downloaded image while the click target opens the original photo. Do not use raw camera-pixel coordinates unless the fragment also includes `&spark-source-size=rawWidth,rawHeight`.
- Prefer simple stable asset names. Final displayed crops belong under `grader/output/assets/...` or `sheet/output/assets/...`; full source-page JPEGs for click-through belong under `grader/output/source-pages/...` or `sheet/output/source-pages/...`; uploaded photos can be reused directly from `grader/uploads/...`.
