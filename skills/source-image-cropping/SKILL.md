...
- For uploaded photo/scan source material, reuse one JPEG source image no larger than 1500px on either side and show localized regions with `#spark-bbox=left,top,right,bottom` fragments instead of cutting multiple copies.
...
5. For uploaded photos/scans, do not run the PDF-style refinement loop. Use at most two simple `view_image` passes: first inspect the full photo, then inspect a rough localized view/crop if needed to choose the bbox. Link the same source JPEG with a `#spark-bbox=left,top,right,bottom` fragment instead of writing separate crop images.
