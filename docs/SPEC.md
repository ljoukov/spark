...
- Worksheet draft generation (from `/spark` chat) is implemented as an Agent run:
  ...
  - Draft worksheet question types use the paper-sheet UI contract: `fill`, `cloze`, `mcq`, `lines`, `calc`, `match`, `spelling`, and `flow`. Use `displayNumber` when a source question needs visible numbering such as `9(a)` or `10(b)`, and the paper-sheet UI should render that label directly instead of silently renumbering the question.
