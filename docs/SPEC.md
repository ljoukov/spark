...
- Quiz completion awards XP by question type (info cards +5, multiple-choice +10, type-answer +12) via the authenticated `/api/session/{sessionId}/update` endpoint with a `quizCompletion` payload. The handler increments `stats.solvedCount`, records idempotent progress under `spark/{uid}/progress`, and returns refreshed stats to the client.
...
- `spark/{uid}/sessions/{sessionId}`: server-only session plans. Each document stores `{ id, title, createdAt: Timestamp, plan: PlanItem[] }` where `PlanItem` may be a quiz (`{ id, kind: 'quiz', title, summary?, icon?, meta? }`), problem (`{ id, kind: 'problem', title, summary?, icon?, meta?, difficulty?, topic? }`), or media clip (`{ id, kind: 'media', title, summary?, icon?, meta?, duration? }`). Session IDs are short human-readable slugs used in URLs. Only SvelteKit server routes read/write this collection; the client receives sessions through the authenticated layout load. Session-specific content lives in subcollections:
  ...
  - `media/{planItemId}`: narration clips with synced slides/captions. Documents follow `SessionMediaDocSchema` (`id`, `planItemId`, `sessionId`, `audio: { storagePath, mimeType?, durationSec, downloadUrl? }`, `slides: Array<{ index, markdown, startSec, durationSec }>`, `captions: Array<{ speaker, text, startSec, durationSec }>`, `metadataVersion`, timestamps). Audio files live in Firebase Storage at `spark/{userId}/sessions/{sessionId}/{planItemId}.mp3` and remain server-authored via the Admin SDK.
  - On a user's first visit to `/code`, if no sessions exist the app shows a modal with three welcome topics (Modular Magic, Binary Sparks, Digital Roots). The chosen template is expanded into `sessions/{sessionId}` plus matching `quiz/` and `code/` subcollections and updates `currentSessionId`.
- `spark/{uid}/state/{sessionId}`: session state document mirrored into SvelteKit responses. Shape `{ sessionId, items: Record<planItemId, { status: 'not_started' | 'in_progress' | 'completed', startedAt?, completedAt?, quiz?: { lastQuestionIndex?: number, serverCompletedAt?: Timestamp, questions: Record<questionId, { status: 'pending' | 'correct' | 'incorrect' | 'skipped', selectedOptionId?, typedValue?, hintUsed?, dontKnow?, firstViewedAt?, answeredAt? }> }, code?: { language: 'python', source: string, savedAt: Timestamp, lastRunStatus?: 'passed' | 'failed' | 'error', lastRunAt?: Timestamp } }>, lastUpdatedAt }`. Only the server reads/writes this collection; browser code receives the state through `load` functions and applies changes by POSTing to `/api/session/{sessionId}/update`, which validates via `@spark/schemas` before persisting.
- Quiz exit UX (web `/code`): clicking the "x" in the quiz progress rail opens a confirmation dialog. Choosing "Quit now" keeps the dialog open in a non-dismissible "Saving your progress..." state until the session update request resolves. On success the client navigates directly to `/code/{sessionId}` (no `invalidateAll()` round-trip); on failure the dialog surfaces a retry button and keeps the learner on the quiz.
...
- Problem submission flow: submit first re-runs every test. Submission is blocked until all tests pass, then a completion payload (including the latest code snapshot and run metadata) is sent to `/api/session/{sessionId}/update`. The handler grants XP once per problem based on difficulty, increments `stats.solvedCount`, records the event under `spark/{uid}/progress`, and the client shows a celebratory modal with XP earned plus an “OK” action that routes back to the `/code` dashboard.
- Media plan steps (intro/outro clips) route to `/code/{sessionId}/m/{planItemId}`. The page renders a timeline-synced player with rounded chevron navigation buttons, a large slide card matching the quiz flash-card dimensions, a single-line subtitle banner (current caption only, no speaker label), and a control bar with icon-only play/pause, speaker toggle, timestamp, and progress scrubber. Slides reuse the non-question card styling; seeking via the buttons or slider also seeks the audio element. The server returns a short-lived signed URL derived from the Firebase Storage object to stream the MP3.
