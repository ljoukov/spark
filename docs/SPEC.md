...
- Bun runtimes default `OPENAI_RESPONSES_WEBSOCKET_MODE=off` and `CHATGPT_RESPONSES_WEBSOCKET_MODE=off` (unless explicitly set) so Spark task workers use SSE transport for OpenAI/ChatGPT Responses.
