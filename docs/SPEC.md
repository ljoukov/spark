...
- Lessons list (web `/code/lessons`): mirrors the `/code` shell with a single panel that lists all lessons newest-first, showing emoji, title/tagline, creation date, derived status (`generating` | `ready` | `in_progress` | `completed` | `error`), and simple step progress; each row links to `/code/{sessionId}`. The user menu includes a `Lessons` entry for quick access.
