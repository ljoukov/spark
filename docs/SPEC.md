...
- `spark/{uid}/sessions/{sessionId}`: server-only session plans. Each document stores `{ id, title, createdAt: Timestamp, plan: PlanItem[] }` where `PlanItem` is either a quiz (`{ id, kind: 'quiz', title, summary?, icon?, meta? }`) or problem (`{ id, kind: 'problem', title, summary?, icon?, meta?, difficulty?, topic? }`). Session IDs are short human-readable slugs used in URLs. Only SvelteKit server routes read/write this collection; the client receives sessions through the authenticated layout load. Session-specific content lives in subcollections:
  ...
  - `code/{problemId}`: code problem metadata (slug, summary, DP notes, examples, solutions) scoped to the user and session. Mirrors the structure in `CodeProblemSchema` and is used by the problem detail page.
