...
- `spark/{uid}/sheetDashboard/current`: the published `/spark/sheets` dashboard state. The document stores `{ schemaVersion: 1, mode: 'sheet_dashboard', headline, summaryMarkdown?, focusNote?, generatedFromRunId?, strengths: Array<{ id, title, summary, evidenceRunIds: string[], subjectKeys: string[] }>, weakSpots: Array<...>, subjects: Array<{ key, label, summary, runIds: string[], averagePercentage?, strongSpots: string[], weakSpots: string[] }>, runAnalyses: Array<{ runId, subjectTags: Array<{ key, label }>, primarySubjectKey?, summary?, strongSpots: string[], weakSpots: string[] }>, updatedAt }`. `/spark/sheets` uses `runAnalyses` for subject filters/tag badges and the top-level arrays for the dashboard detail panel.
