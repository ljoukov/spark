...
- Quiz content (prompts, hints, explanations, info-card bodies, and choice labels) accepts lightweight Markdown authored in the schema. The SvelteKit loader renders it with `marked` (GFM enabled, soft line breaks) plus the KaTeX extension so `$...$` / `$$...$$` formulas display inline or block-level before hydrating the quiz components. When no Markdown is present the components fall back to the existing plain-text rendering, so data authors do not need to escape anything manually.
...
- On a user's first visit to `/code`, if no sessions exist the server fetches available welcome templates from `spark-admin/templates/sessions`. Each template document provides `{ id, title, summary?, plan, tagline, emoji, topic, key? }` plus child collections for `quiz/`, `code/`, and `media/`. The modal lists the templates (sorted by title); picking one clones the template into `spark/{uid}/sessions/{sessionId}` with matching subcollections, copies any narration media into the user namespace, and updates `currentSessionId`.
...
- Advent calendar (web `/code/events/advents`): static 25-day grid with large emoji artwork and hard-coded coding hooks for each day. Tiles for days greater than the current day-of-month stay locked (no modal opens); unlocked/today tiles open an in-page detail sheet with the prompt text and cosmetic prize copy on Day 25. No links navigate away from the page; reachable via the user menu entry “Advent of Code”.
- Advent advent sessions (web `/code/events/advents`, days 1–5): the first five doors include fully hard-coded session bundles (plan, two problems, warmup quiz with 15 questions, wrap quiz with 7 questions). Content is Python-first (no C/C++ pointer jargon) and quizzes explain “two-pointer” as dual indices in Python. The detail sheet shows plan items, quiz counts, and problem difficulty; a “Copy session JSON” action copies the bundle. Later days remain prompt-only and stay locked past the current day.
