...
- Uploaded-work grading (from `/spark` chat) is implemented as an Agent run:
  ...
  - Per-problem grading feedback is line-by-line against that numbered student transcript so each student statement gets a corresponding note.
...
- `spark/{uid}/tutorSessions/{sessionId}`: per-problem tutor review sessions seeded from grader problem reports. The session doc tracks `{ workspaceId, status, source, title, preview?, focusLabel?, latestDraftRevision?, createdAt, updatedAt, completedAt? }`, while the paired workspace stores `context/problem.md`, `context/student-transcript.md`, grading reference markdown, and `state/review.json` containing the structured review threads (`transcriptLines[]`, per-thread message history, status, resolvedAt?`). Tutor sessions are review-first rather than chat-first: line-by-line annotations seed transcript threads, only actionable whole-problem feedback seeds problem-level threads, already-correct comments can be pre-resolved, and the session becomes `completed` when every actionable thread is resolved.
