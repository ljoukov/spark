...
- `/spark/sheets` lists both draft worksheet runs and graded sheet runs launched from chat. Each card uses the same paper-sheet header/footer visual language as the full worksheet, plus a concise title, a `Marks` summary in the header box (`awarded / total` with percentage once graded, or a pending state against the total before grading), phase-aware status (`Queued`, `Preparing`, `Ready to solve`, `Grading`, `Graded`), and rendered Markdown summary instead of raw process logs or machine identifiers.
