...
- Uploaded-work grading (from `/spark` chat) is implemented as an Agent run:
  ...
  - Student transcription must be complete and faithful, then normalized into numbered statements/sentences per problem (in source order) rather than retold; variable names, formulas, terminology, and method choice are preserved as closely as possible except for numbering, line-break cleanup, and obvious spelling fixes that do not change meaning. When the learner answers on the same page as the original task (for example fill-in-the-blank work on the worksheet itself), the transcript must clearly separate original prompt text from the student's contributed response so later grading/review never confuses the two.
  ...
  - Per-problem grading feedback is line-by-line against that numbered student transcript so each student statement gets a corresponding note. The tone should be empathetic and student-facing: acknowledge sensible partial thinking or correct structure before naming the missing piece, prefer methodological nudges/rules of thumb before full answers, and do not treat a near-miss as resolved when the actual task is still unmet.
  ...
  - On completion, the runner parses `grader/output/run-summary.json`, prefers `presentation.summaryMarkdown` over the raw `done({summary})` text, and patches the grader run document with presentation + totals + per-problem summary metadata for `/spark/grader`.
...
- Signed-in experiences live under `/(app)/(signed)` with a shared shell (user avatar menu showing display name + email/guest label and a copy button that copies `Name`/`Email`/`UserID` lines to the clipboard (omitting missing fields), Chats / Lessons / Grader runs shortcuts, theme picker, Firebase auth sync) reused by `/spark` and `/spark/lesson`.
- `/spark` is the signed-in home for the Spark AI Agent chat and includes quick links to Lessons (`/spark/lessons`), Grader runs (`/spark/grader`), and Agents (`/spark/agents`). The current conversation can also be reopened directly with `/spark?conversationId={conversationId}`. If the referenced conversation cannot be rendered from its saved Firestore payload, Spark clears the broken `conversationId`, shows an inline error, and returns the user to a fresh chat instead of leaving them on an empty thread. When chat launches lesson creation or uploaded-work grading, the assistant message renders a live status card in-thread instead of dumping raw tool-call machine data; the card links directly to the lesson/run plus the corresponding list page and updates from Firestore while the background run is still executing.
- `/spark/chats` lists the user's 50 most recent Spark AI Agent conversations newest-first, supports client-side search plus type filters (`All`, `Chat`, `Grading`, `Quiz`, `Lesson`), and groups results into Today, Yesterday, Last 7 days, and Last 30 days (with an Older fallback if needed). Each row links back into `/spark` with the matching `conversationId`.
- `/spark/grader` lists grading runs launched from chat. Each run card uses a concise user-facing title, a compact stats line (marks / problems / percent), and rendered Markdown summary text instead of raw process logs or machine identifiers. Opening a run (`/spark/grader/{runId}`) shows the same presentation summary plus totals, paper metadata/source links, and per-problem grading rows; opening a problem (`/spark/grader/{runId}/{problemId}`) shows the generated markdown sections for problem statement, official problem statement, official solution, a numbered student transcript, grading notes, line-by-line annotations keyed to that transcript, and overall feedback.
- `/spark/sessions` lists per-problem tutor review sessions launched from grader problems. Opening a session (`/spark/sessions/{sessionId}`) uses a PR-style review surface anchored on the numbered student transcript: the right rail shows the official problem formulation plus collapsible reference notes, each transcript line carries its own reviewer thread, problem-level comments live in a separate thread section, and every open thread has its own reply box for the student. Tutor replies are non-streaming review comments submitted per thread (not a single interactive chat composer), resolved threads show a green check, non-actionable "already correct" comments seed as resolved instead of requesting a reply, and overall feedback only becomes its own student-response thread when it adds actionable whole-problem work beyond the line-level comments. Reviewer language should stay warm and specific: it should validate sensible partial thinking, prefer methodological direction or rules of thumb before giving the answer away, keep the original task distinct from the student's attempt, and only resolve a thread once the actual asked task has been met. The session completes automatically once every actionable line-level and problem-level thread is resolved. If a tutor reply stalls and leaves a thread in `responding`, Spark auto-recovers the session on the next load with a fallback reviewer reply so the student is not stuck in a permanent reviewing state. If a tutor session URL is opened after that session has been deleted, Spark redirects to the most recently updated remaining tutor session, or back to `/spark/sessions` when none remain.
...
- Opening a run (`/spark/agents/{runId}`) shows the run details:
  ...
  - The user-facing summary also renders Markdown. Metadata shows the agent ID, workspace ID, created timestamp, updated timestamp, elapsed duration, status timeline, and a Stop button (only while `status` is `created`/`executing` and `stop_requested` is not set; after requesting stop, the UI shows a “stop requested” badge).
...
- `spark/{uid}/tutorSessions/{sessionId}`: per-problem tutor review sessions seeded from grader problem reports. The session doc tracks `{ workspaceId, status, source, title, preview?, focusLabel?, createdAt, updatedAt, completedAt? }`, while the paired workspace stores `context/problem.md`, `context/student-transcript.md`, grading reference markdown, and `state/review.json` containing the structured review threads (`transcriptLines[]`, per-thread message history, status, resolvedAt?`). Tutor sessions are review-first rather than chat-first: line-by-line annotations seed transcript threads, only actionable whole-problem feedback seeds problem-level threads, already-correct comments can be pre-resolved, and the session becomes `completed` when every actionable thread is resolved. Transcript lines must keep original prompt text separate from student-written content when both appear on the same worksheet page, and reviewer follow-ups should resolve only after the student has met the actual task rather than merely approaching it.
...
- `spark/{uid}/graderRuns/{runId}`: grading run metadata launched from `create_grader`. Each document stores `{ id, agentId, workspaceId, conversationId?, userPrompt?, olympiadKey, olympiadLabel, summaryPath, problemsDir, sourceAttachmentIds?, sourceAttachmentCount?, status: 'created'|'executing'|'stopped'|'failed'|'done', paper?, presentation?: { title?, summaryMarkdown? }, totals?, problems?, resultSummary?, error?, createdAt, updatedAt, completedAt? }`. `olympiadKey` / `olympiadLabel` are legacy seed-title fields for the pre-summary launch state; `paper` captures inferred paper metadata/URLs (including `contextLabel` when known), `presentation` carries the UI-facing title/summary markdown, and `problems` contains per-problem summary rows (`id`, `index`, `title`, marks, verdict, `filePath`) used by `/spark/grader`.
...
- Lessons list (web `/spark/lessons`): mirrors the `/spark/lesson` shell with a single panel that lists all lessons newest-first, showing emoji, title/tagline, creation date, derived status (`generating` | `ready` | `in_progress` | `completed` | `error`), and simple step progress; each row links to `/spark/lesson/{sessionId}`. The user menu includes `Chats`, `Lessons`, and `Grader runs` entries for quick access. The page also shows all welcome templates with a “Start lesson” action that posts to `/spark/lessons?/start`; submitting clones the template into the user’s sessions (same flow as first-time welcome) and redirects to the new `/spark/lesson/{sessionId}` even when other sessions already exist.
...
- LLM providers: Gemini (Vertex AI) and OpenAI (Responses API). The shared wrapper defaults OpenAI calls to `gpt-5.2` with `reasoning: { effort: "medium" }` when an OpenAI model is selected. Spark chat (`POST /api/spark/agent/messages`) uses `chatgpt-gpt-5.4` with `thinkingLevel: "medium"`, while Spark run-agent execution (`packages/llm/src/agent/sparkAgentRunner.ts`) uses `chatgpt-gpt-5.4` with `thinkingLevel: "high"`. Free-text grading uses `gemini-flash-latest`.
