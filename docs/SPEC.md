...
- `spark/{uid}/progress/{docId}`: server-managed bookkeeping for awarded milestones (e.g., quiz completions). Docs store `{ sessionId, quizId, planItemId, xpAwarded, createdAt }` to keep XP increments idempotent.
