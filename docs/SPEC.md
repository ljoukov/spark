...
- Uploaded-work grading (from `/spark` chat or from a ready-to-solve draft sheet) is implemented as an Agent run:
  ...
  - Per-question worksheet review notes for unresolved or incorrect work must be empathetic, student-facing, and shaped for the Spark-owned sheet response modal: acknowledge sensible partial thinking or correct structure before naming the missing move, prefer methodological nudges/rules of thumb before full answers, and end by setting up the next repair the learner should write. The inline note must not reveal the model answer, mark scheme, final corrected sentence, complete method, or the missing final concept in a way that replaces the student's work; it should start with a cue, contrast, or next step that keeps the student doing the learning. Add `replyPlaceholder` when a focused response prompt would help the learner close the gap on that particular problem. Correct/resolved items do not need visible review notes.
...
- Interactive follow-up also happens on `/spark/sheets/{sheetId}`. There is no separate `/spark/sessions/{sessionId}` product surface. When the student opens a question response and sends a reply, Spark creates a hidden sheet-interaction session and runs one direct question-scoped close-the-gap chat turn; it does not launch the general tutor agent, create a tutor workspace, expose filesystem tools, or pass whole-sheet workspace context into the model. The direct chat prompt contains only the selected question, locked submitted answer, inline review note, current thread, latest student reply, and a tightly scoped private reference excerpt when one can be matched. It must not paste the whole mark scheme, official solution, or whole-sheet feedback into the turn; the tutor should guide first and reveal a final/model answer only when pedagogically appropriate after the student has tried or directly needs it. Each assistant turn also returns a discrete `gapBand` (`large_gap`, `medium_gap`, `small_gap`, or `closed`) that the response modal maps onto sheet-style red, amber, and green shading in both light and dark mode. Each worksheet question has at most one open thread. Resolved questions show a done state, collapse by default, and replace the live composer with a right-aligned `ask followup` action that reopens the resolved thread when the student wants another pass; question chrome and feedback-card color should still follow the awarded-mark tone, with green reserved for full marks, amber/review tone for partial marks, and incorrect/red tone for zero marks. Open questions keep the per-question review note inline, but the inline card only exposes a `response` action instead of a reply composer. That action opens a Spark-owned full-screen, blurred-backdrop response modal focused on closing the gap for that particular problem: the modal keeps the problem statement and locked student answer visible, then shows the review thread, attachment-capable Spark chat composer, and runtime state in the main response area. On desktop the dialog uses almost the full viewport height with about `64px` top and bottom breathing room; on mobile it is edge-to-edge. After the student posts a message, the thread scrolls that message to the top and the following assistant/status bubble reserves near-viewport height like the main `/spark` chat so there is stable space for the reply. Tutor and student chat bubbles plus the live reply textarea use the standard `16px` reading size on this surface. The modal composer includes a leading `+` menu for attaching the same supported image/document types as chat replies; the student can send text only, files only, or both, and attached files render inline in the thread once the turn is submitted. On narrow screens, the inline feedback note header keeps the review pill pinned left while the live status stays right-aligned with the chevron in its own trailing slot, and the response modal becomes a full-height single-column problem-then-response flow. While a reply is active, the matching inline feedback card and the response modal show chat-style runtime state and lock the composer; the composer animates from the full textarea into a compact circular loading shell, with draft/placeholder cleared, attach/input affordances hidden, and the send control shown as a spinner-only outlined circle. Reviewer language should stay warm and specific: it should validate sensible partial thinking, prefer methodological direction or rules of thumb before giving the answer away, keep the original task distinct from the student's attempt, and only resolve a thread once the actual asked task has been met.
