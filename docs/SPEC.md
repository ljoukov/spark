...
- Spark is an effort comprising two apps:
  ...
  - Spark AI Agent (web, Phase 1)
    ...
    - Dragging and dropping supported images/PDFs onto the `/spark` chat view attaches them through the same upload path as picker/paste flows.
    - While dragging supported files over `/spark`, the UI shows a full-screen HTML drop overlay: a dashed rounded panel styled like the “Start a new conversation” surface, three colorful cards (code/image/document), a down-arrow cue, title `Add anything`, and subtext `Drop documents and images here to add to Spark`; the backdrop reuses Spark’s blob color atmosphere, and the panel animates in/out with a short upward slide while keeping a more solid interior fill for readability.
