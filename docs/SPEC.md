...
- Worksheet draft generation (from `/spark` chat) is implemented as an Agent run:
  ...
  - While solving a draft sheet, the client keeps the visible answer state local immediately and persists batches to `sheet/state/answers.json` on a sensible cadence: at most about every 10 seconds while editing, and immediately on input focus loss, window blur, visibility/pagehide/unload, or before grading starts.
  ...
  - For uploaded worksheet / exam pages, the draft-sheet agent must run an extraction-first and visual-check workflow: `extract_text` over the relevant uploads, `extract_pdf_images` as the deterministic embedded-raster first pass for source PDFs with named figures/diagrams/photos, `pdf_to_images` for relevant pages only when embedded images are insufficient or page context is required, and `view_image` on every page/crop needed to preserve source structure faithfully. For source PDFs, `pdf_to_images` calls should use explicit 1-based `pageNumbers` chosen from extracted page headings or grep context. PDFs over 12 pages require page numbers and must not be rendered all at once.
  - In grader runs with uploaded question paper and mark scheme files, source visual/table handling is completed before scoring polish, model-answer wording, optional reference enrichment, or thumbnail-summary work. Model answers for wrong responses come from the uploaded mark scheme and source prompt unless the user explicitly asks for additional official worked examples or examiner-report context.
...
- Uploaded-work grading (from `/spark` chat or from a ready-to-solve draft sheet) is implemented as an Agent run:
  ...
  - Model-answer requests are not grading requests: wording such as "model answers", "full-mark answers", "answer key", "worked solutions", "using the mark scheme", or "make sure they are full marks" should stay in the normal chat answer path unless the learner explicitly asks to grade submitted/student work. The router must not treat the noun phrase "mark scheme" or "full marks" as the verb "mark".
  ...
  - Upload inventory, workspace file reading, primary transcription, final scoring synthesis, and worksheet assembly stay on the main agent. Generic `spawn_agent` may be used only for bounded sidecar work such as official-reference lookup/verification when online lookup is allowed or visual localization proposals for ambiguous source images. For long handwritten papers, the agent writes `grader/output/sheet-plan.md` after transcription and before crop refinement or scoring; the dedicated `score_answers_with_fresh_agent` tool then scores one root question or short contiguous range from focused source/student/mark-scheme excerpts plus any figure/table summaries needed for scoring. It returns per-question results inline while also writing scoring audit files and may also return concise `modelAnswer` text for incorrect or partial items. The main agent uses those returned results directly, including any returned `teacher-review` status, reconciles totals, assembles the report, and publishes without rereading every scoring file, regrading from images, or deriving separate model answers unless a named question is missing or validation points to it. Long handwritten publish runs enforce this order: after a long handwritten transcript exists, including raw extract files such as `transcription-raw.md` or `student-extract.md`, crop polishing/publish validation waits for `sheet-plan.md`; after the plan exists, crop validation, artifact writes, source-fidelity audit, and publish wait for `score_answers_with_fresh_agent`; after scoring is complete, non-JSON crop validation, polishing, search, and regrading are blocked until first-pass `grader/output/sheet.json` and `grader/output/run-summary.json` exist. The grader writes those JSON artifacts with `write_json_workspace_file`, passing the complete JSON object in `jsonText`; the tool rejects invalid JSON and empty `{}` grader artifacts before writing. It then calls `validate_grader_artifacts`, fixes named schema/crop/source-fidelity blockers, records required PDF crop validation, and calls validation again before source-fidelity audit so invalid JSON, schema errors, score mismatches, missing PDF crop validation, non-guarded image paths, or missing long-paper scoring batches are caught without publishing. Grader runs must not look up grade boundaries or examiner reports unless the user explicitly requested those external references; with uploaded question paper / mark scheme files, the source-faithful sheet is completed first, and single component papers are not converted into full qualification grades. Printed source papers/photos and PDF-derived worksheets require the dedicated `validate_source_fidelity_with_fresh_agent` tool before publish; the tool writes per-scope reviews under `grader/output/source-fidelity-audits/` and refreshes aggregate `grader/output/source-fidelity-audit.md`, split by page or root question for long sources with the relevant source reference markdown and rendered page images passed in `sourcePaths`, and must check verbatim wording, visible answered/partial/blank item coverage, numbering/badges, figure/table/layout preservation, and answer-evidence attachment before publication. If that audit fails, the grader patches all blockers for the same page/root in one pass and runs at most one follow-up audit before using `review_run_progress_with_fresh_agent` or surfacing a concrete blocker. Broad focus wording such as "first 4 paragraphs" is not permission to omit later visible answered or answer-bearing items unless the user explicitly excludes named question numbers. After a large extracted reference returns only an outline or omitted content, the agent must use grep or exact line ranges instead of repeating unbounded reads. The dedicated `review_run_progress_with_fresh_agent` tool runs a bounded fresh-context introspection pass over sanitized completed local tool traces when repeated crop attempts, repeated validation failures, budget warnings, or recurring publish errors suggest the run is on the wrong path; it returns continue/switch/stop advice but does not grade or assemble artifacts. Scratch image inventory paths such as `grader/output/pdf-images/...` are not worksheet assets; final displayed PDF crops must be written under `grader/output/assets/...` or `sheet/output/assets/...` before linking, while full rendered source-page JPEG click targets live under `grader/output/source-pages/...` or `sheet/output/source-pages/...`. Uploaded photos/scans reuse a JPEG source image with `#spark-bbox` viewport fragments instead of repeated crop files. Each final linked PDF figure/image crop used in the worksheet requires its own `validate_crop_with_fresh_agent` validation pass with `view_image`, question context, explicit `expectedContent`, explicit `duplicatedTextToExclude` when surrounding prompt/caption/table text is rendered separately, reviewer-visible text transcription, and a pass/fail judgement; `expectedContent` lists only printed/visible source content, not inferred answers, mark-scheme facts, or unprinted labels/headings. Photo viewport links record their source JPEG, bbox, and two-pass `view_image` check in the plan/source-fidelity notes instead of `crop-validation.md`. If an expected-content mismatch is the only failure on an otherwise complete visual, the agent corrects `expectedContent` and revalidates once instead of recropping. Related subpanels inside the same official embedded figure/table image are target context, not neighbouring-question noise, when all expected content is visible. Padding a PDF crop that already passed validation does not require another validation pass when the padding only adds a white border; `crop-validation.md` records the final padded path and the passed source validation. `grader/output/crop-validation.md` records each final linked PDF crop path with `fresh-context subagent checked: yes`, the reviewer-visible text, and the pass/fail outcome. The main agent always consolidates final outputs before optional feedback enrichment.
  ...
  - When grading starts from a draft sheet, it is a digital-answer grading path rather than a new grader agent type: the existing grader must read `sheet/output/draft.json` and `sheet/state/answers.json`, preserve that student-facing structure, grade the saved answer values directly, skip OCR/transcription/source-crop work unless validation names a source-reconstruction blocker, and publish the final graded worksheet against the same run record (`sheetPhase: solving -> grading -> graded`).
  ...
  - Per-question worksheet review notes for unresolved or incorrect work must be empathetic, student-facing, and shaped for the Spark-owned sheet close-gap answer builder: acknowledge sensible partial thinking or correct structure before naming the missing move, prefer methodological nudges/rules of thumb before full answers, and set up the next repair the learner should build. The inline note must not reveal the model answer, mark scheme, final corrected sentence, complete method, or the missing final concept in a way that replaces the student's work; it should start with a cue, contrast, or next step that keeps the student doing the learning. Add `replyPlaceholder` when a focused repair prompt would help the generated answer-builder path, and include `modelAnswer` for unresolved/incorrect questions when a concise mark-scheme answer is available so the sheet close-gap generator can build correct guided steps. Correct/resolved items do not need visible review notes.
  ...
  - When a figure/photo/diagram matters, the grader should render it from source-backed pixels so `/spark/sheets/{runId}` can show a clickable in-sheet figure. Source references and transcription markdown are audit trails only, not substitutes for the visible worksheet: any prompt/group prompt that names or points to a diagram, figure, graph, chart, map, network, photo, or table must include the linked crop, photo viewport, or Markdown table near that text. SVG is not a worksheet output format. Markdown/LaTeX may be used only for simple horizontal/vertical grids, boxed arrays, and text/number layouts; angles, geometry diagrams, 3D solids, graphs, charts, maps, networks, photos, apparatus, option diagrams, and other complex visuals must use source-backed images. PDF embedded raster extraction should be tried when useful, with tiny repeated strip artifacts filtered out and useful outputs freshly validated. PDF pages should still be rendered to high-resolution page images by default for vector/layout-sensitive figures, capped to a safe maximum dimension; displayed PDF figures are cropped into `grader/output/assets/...` / `sheet/output/assets/...`, while the image click target opens the full rendered PDF page as a JPEG no larger than 1500px on either side under `grader/output/source-pages/...` / `sheet/output/source-pages/...`. Uploaded photos/scans instead reuse one JPEG source image no larger than 1500px on either side and display localized regions by attaching a `#spark-bbox=left,top,right,bottom` fragment to the image target while the link target opens the full photo; bbox coordinates are measured on the EXIF-oriented, 1500px-normalized displayed JPEG, not raw camera pixels, unless the fragment also includes `spark-source-size`. Those photo viewports use at most two simple `view_image` checks rather than the PDF crop-refinement loop. Linked-original/source-PDF instructions are allowed only when a real render/crop attempt failed or the pixels are unavailable, and that reason is recorded in the sheet plan plus source-fidelity audit. PDF crop refinement uses one clean rectangular crop per target figure, rejects surrounding question text/mark text/answer lines/page borders/next-question content, and prefers a small safe white margin over clipping; clipping required information is worse than leaving slight extra whitespace. If a figure region contains a transcribable text/numeric table, the table should be represented as Markdown in the worksheet even if a crop is also included for surrounding visual context. Tables and figures should be placed at the nearest source-faithful level: parent `group.prompt` when shared by subparts, child prompt when specific to one subquestion.
  ...
  - `grader/output/run-summary.json` also carries a grader-specific `presentation` block (`title`, `summaryMarkdown`) for direct UI rendering. It must stay human-facing and concise: no IDs, file paths, tool/process narration, `Unknown` filler strings, or generic lead-ins. `presentation.summaryMarkdown` is thumbnail body data and should be one compact sentence or two short fragments that avoid repeating title, subject, level, marks, percentages, created dates, or footer text.
...
- `/spark/sheets` lists both draft worksheet runs and graded sheet runs launched from chat. The page streams the route shell immediately and shows skeleton cards while the server resolves worksheet runs, artifact previews, analyses, and gaps. If a grader run failed only because of a transient upstream model-service error after valid `grader/output/run-summary.json` and `grader/output/sheet.json` artifacts were written, the server recovers the run from those already-written artifacts and marks it graded instead of showing a failed thumbnail or retrying the LLM. It exposes fast subject filters derived from each sheet's preview subject, or from a persisted per-run sheet analysis when one already exists, normalizing compound labels down to stable broad subjects (for example `Combined Science: Trilogy Physics` becomes `Physics`) so color mapping stays consistent across filters and cards. Sheet colors come from the shared stable Apple-style subject palette used by both published worksheet artifacts and the sheet list: Biology green, Mathematics blue, Chemistry purple, Physics indigo, Geography teal, Science mint, English pink, History/Religious Studies orange, Economics/Business yellow, and Computer Science/General gray, with paired light/dark tokens for the list UI. The same canonical subject labels appear on sheet cards so learners can scan or narrow the list quickly. When the subject filter is not `All subjects`, the page also shows a second filter row for practice-gap type (`All`, `Knowledge gap`, `Misconception`, `Oversight`) followed by one horizontally scrolling row of generated gap cards before the sheet thumbnails. Gap cards inherit the subject color and layer a stronger type color (`knowledge_gap` green/teal, `misconception` rose, `oversight` amber), fit roughly three cards on desktop, use about 80% viewport width on mobile, and leave the next card visibly peeking so horizontal scrolling is obvious. Sheet thumbnails use a fixed four-column desktop grid with responsive three/two/one-column breakpoints so filtered sets with only one or two sheets still occupy card-width columns instead of stretching across the page. In dark mode the filters and sheet cards should borrow the real worksheet dark surfaces and text ramp (`#17142a` / `#201c39` / `#1d1934`, divider `#3a3258`, text `#e4dff5` / `#a89ec4`) so the page reads like a sheet-native surface rather than light cards dropped onto the dark app shell. Sheet cards keep the paper-sheet header/footer visual language, with identity split into explicit regions: eyebrow (`level · subject`), concise title, short subtitle, a `Marks` summary in the header box (`awarded / total` with percentage once graded, or a pending state against the total before grading), phase-aware status (`Queued`, `Preparing`, `Ready to solve`, `Grading`, `Graded`), one rendered Markdown body summary sentence/paragraph, optional per-run development signal chips when persisted analysis is available, and a quiet footer provenance line. Titles and subject labels must not be repeated in the footer. The body summary must also avoid repeating the header/footer/marks; for official exams or competitions it should prefer known grade, prize, medal, or percentile outcomes when suitable, otherwise common examiner mistakes made/avoided or another concise actionable score driver. `sheet-preview__signals` is capped at three items and focuses only on topics, areas, or skills for development rather than praise.
- Opening a practice gap (`/spark/gaps/{gapId}`) uses the same stripped standalone signed layout as sheet detail pages: no top app navbar is rendered, and the only route-level close control is the shared floating round `×` button that returns to `/spark/sheets`. The gap page renders only the guided-question presentation on a full-viewport sheet-like surface with no rounded page container and no mode selector. `Guided` mode uses persisted `presentations.v17` data to render a worksheet-style guided-question path: the main gap question renders normal Markdown with KaTeX for formulas/calculations, while inline field questions, short field inputs, expected answers, and field-grader hints remain plain single-line text with no Markdown or LaTeX, using Unicode symbols where useful. The first question phase goes directly from the gap header to a `Build the answer` section bar. Each short field is checked on blur, Enter, or focus movement through `gemini-flash-latest` with prior attempts and other guided replies included so hints follow four levels: broad relationship-building, closer choice/options, very close answer cue, then a non-copyable reveal of the key idea. After all short fields are filled, the student sees a compact memory chain, writes a full GCSE answer in a multi-line input, receives `gemini-flash-latest` annotated-text feedback, retries until full marks, then sees the model answer and a `Done` action that only closes the gap route.
- Opening a sheet (`/spark/sheets/{sheetId}`) always shows the paper-sheet UI inside a dedicated sheet-only layout, with immediate route-transition feedback while the sheet page is being opened. The worksheet surface follows the signed-in shell theme (light/dark) while preserving the sheet-specific accent color from the stable subject palette, but once the run has published a draft/report artifact (or a legacy completed run points at one) the page drops straight into the worksheet itself with no shared header, no extra sheet-page header or summary card, no separate reference-notes section, and only a floating round `×` close button leading back to `/spark/sheets`. The close button must remain usable immediately, even while the sheet body or route transition is still resolving. The worksheet column itself is capped at `1024px`, centered in the viewport, and gains extra vertical padding once the viewport is wider than that cap. While sheet generation or grading is still running, the page can stay in a minimal queued / preparing / grading / failed / stopped placeholder state until the worksheet body is published; the browser refreshes sheet state through the authenticated `/api/spark/sheets/{sheetId}/state` route with long-running backoff, and does not render unpublished in-progress workspace artifacts directly from Firestore. Draft sheets render as editable worksheets: the student can type answers directly into the paper-sheet UI, the page updates the local answer state immediately, autosaves pending answers into `sheet/state/answers.json` at most about every 10 seconds while editing, flushes on input focus loss and page/window exit signals, and a bottom `Grade` button starts grading after the pending answers are saved. Question badges render `badgeLabel` when it is present, otherwise they fall back to `displayNumber`; section id pills are hidden on the Spark sheet route so meaningless internal ids do not appear as paper structure. When a numbered source question owns shared context and subparts, the paper-sheet UI should render that parent as a grouped question block: the parent number and shared Markdown/table stay attached to the subparts instead of drifting to section scope. Once the graded worksheet is available, the same page renders the published report directly in the paper-sheet UI with the student's submitted answers locked in place, and locked long-answer solutions move into their own full-width row beneath the prompt so they align with the feedback cards that follow. `lines` questions with `renderMode = "markdown"` stay editable while solving and only switch to rendered markdown after inputs are locked. If a worksheet review summary is present, it appears first inside the body as a full-width paper-sheet review band directly below the header and stays concise: the visible content is the main summary line, the numeric score, and one primary supporting paragraph, without a secondary muted follow-up note. When a worksheet hook/introduction is also present, it follows beneath that review band before the first question section; on narrow screens the same order is preserved without introducing a side rail. The sheet uses a `16px` base reading size across regular worksheet text and controls, and the ruled long-answer rows scale their line spacing with that type size; rendered long-answer markdown keeps a slightly looser line rhythm so multi-line submissions read cleanly inside the ruled box. Source tables render as attached paper-style tables with clear spacing from their label, clickable image crops render without an "Open image" overlay badge, and compact inline calculation inputs expand to show their full value when space allows. When original or official source material is available for a published worksheet, the page shows an `Original materials` card after the worksheet footer rather than inside the worksheet body; links can include the question paper PDF, mark scheme/official-solutions PDF, markers' report or original upload, and request chat, deduping upload-manifest files against identical reference storage paths, and each opens in a new tab while the worksheet footer remains a concise paper identity line. On wider screens each question keeps a dedicated number / prompt / marks row, while grouped multipart parents use the same number column for the shared context and visually nest their child subparts beneath it. On narrow screens the number badge and mark label float around the prompt before any answer box or feedback continues below. If the grader cannot determine any submitted long-answer text for a locked row, that answer box should show a short neutral placeholder instead of an empty lined surface.
- Interactive close-gap work also happens on `/spark/sheets/{sheetId}`. There is no separate `/spark/sessions/{sessionId}` product surface and no chat UI in the sheet close-gap flow. Opening a question's `Close the gap` action updates the address to `/spark/sheets/{sheetId}#{questionId}/{stage}` so the selected question and current answer-builder stage are copy/pasteable. The sheet remains visible behind a blurred modal backdrop; the modal body reuses the same guided answer-builder structure as `/spark/gaps/{gapId}` rather than showing a threaded conversation or composer. Spark lazily generates and persists one `guidedPresentation` per sheet feedback thread, aiming for 1-2 guided fields per available mark for science questions, and pre-fills only guided fields whose ideas are already clearly shown in the student's submitted answer. The full written answer page always requires the learner to write a new answer. The modal header includes `<` and `>` controls that move backward and forward only through stages the learner has already visited; browser Back/Forward also moves through the staged hash history. Guided field answers, attempts, the written answer draft, final grading result, phase, and visited-stage index persist in Firestore under the hidden sheet interaction session so reopening the same close-gap item restores the learner's work. The close-gap UI blocks copy/cut/paste and disables ordinary text selection outside editable fields so model-answer and hint text are not copy/paste-ready. Each worksheet question has at most one guided close-gap state; question chrome and feedback-card color should still follow the awarded-mark tone, with green reserved for full marks, amber/review tone for partial marks, and incorrect/red tone for zero marks.
...
- `spark/{uid}/tutorSessions/{sessionId}`: hidden sheet-interaction sessions seeded from worksheet reports once the student replies on a sheet. The session doc tracks `{ workspaceId, status, source, title, preview?, focusLabel?, activeTurnQuestionId?, reviewState, error?, createdAt, updatedAt, completedAt? }`; `workspaceId` is retained as a legacy identifier, but close-the-gap sheet replies do not create or read tutor workspace files. `reviewState` contains the structured worksheet review state (`sheet`, `answers`, `review`, and per-question thread status/history), with each review thread optionally carrying `gapBand` so the client can shade the close-the-gap modal by remaining-gap severity. Student replies are appended directly to the relevant thread, Spark runs one direct focused chat model call, then appends the assistant reply and marks that question `open` or `resolved`.
...
- `spark/{uid}/gapsFinder/state`: server-owned state for the gaps-finder background agent. It stores `{ schemaVersion, status: 'idle'|'running'|'failed', leaseId?, startedAt?, updatedAt, lastCompletedAt?, lastWorkspaceId?, lastError? }` and prevents concurrent runs for the same user. New graded sheets are queued under `spark/{uid}/gapsFinder/state/pendingRuns/{runVersion}` with `{ runId, runVersion, completedAt?, queuedAt, status }`; completed queue entries are recorded under `processedRuns/{runVersion}` with `{ processedAt, gapCount, candidateCount, workspaceId? }`. `runVersion` combines the grader run id with the grading completion timestamp so a later regrade can be processed once while duplicate submissions of the same paper still dedupe at the gap level. Each gaps-finder run writes a workspace under the standard Spark agent workspace root with `brief.md`, `request.json`, `skills/gap-finder/SKILL.md`, `gaps/input/new-runs.json`, `gaps/input/existing-gaps.json`, and one worksheet report JSON per queued run. Operator runs can set `forceUiData` to refresh `presentations.v17` for every active gap after the pending queue is drained; this refresh also rewrites long legacy gap document ids to compact `{subjectKey}-{shortHash}` ids.
- `spark/{uid}/gaps/{gapId}`: active practice gaps generated from low-score worksheet questions. Documents follow `SparkLearningGapSchema` and include `{ schemaVersion, status, type: 'knowledge_gap'|'misconception'|'oversight', title, cardQuestion, shortRationale?, subjectKey, subjectLabel, dedupeKey, severity, source, steps, presentations?, createdAt, updatedAt }`. New gap ids use compact `{subjectKey}-{shortHash}` document ids for short `/spark/gaps/{gapId}` links. `source` points back to the grader run version and worksheet question, including awarded/max marks when available. `steps` are an ordered learning flow of `free_text`, rare `multiple_choice`, `model_answer`, and `memory_chain` items; every step may carry a short student-facing `label` for source/fallback context. `presentations.v17` stores the guided answer builder (`question`, optional `instructions`, `questions[]` with `question`/`expectedAnswer`/optional `hint`, `memoryChain`, optional `answerPrompt`, `modelAnswer`, optional `markScheme`, and optional `maxMarks`). The presentation `question` is authored as Markdown with KaTeX support when needed for formulas, calculations, or structured source detail; inline field `questions[]`, `expectedAnswer`, and `hint` remain plain single-line text. Existing gaps without `presentations.v17` can still render through deterministic fallbacks, but new and backfilled gaps should persist only `presentations.v17`. Hidden sheet interaction sessions may also store per-thread `guidedPresentation` plus `guidedState` (`phase`, `maxVisitedPhaseIndex`, short-field answers/results/attempts, last-checked values, and written-answer draft) for the `/spark/sheets/{sheetId}` close-gap modal.
