...
- Uploaded-work grading (from `/spark` chat or from a ready-to-solve draft sheet) is implemented as an Agent run:
  ...
  - Worksheet question types are limited to the paper-sheet UI contract: `answer_bank`, `fill`, `cloze`, `mcq`, `lines`, `calc`, `match`, `spelling`, and `flow`. When a problem does not fit neatly into an objective worksheet type, the grader should use `lines`.
