...
- Opening a run (`/spark/agents/{runId}`) shows the run details:
  - The run header is prompt-first: a `Prompt` label, the full prompt rendered in standard body text underneath, and a top row that keeps `Copy prompt`, `Download zip`, and the status pill together.
