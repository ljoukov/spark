...
- Worksheet draft generation (from `/spark` chat) is implemented as an Agent run:
  ...
  - For uploaded worksheet / exam pages, the draft-sheet agent must run an extraction-first and visual-check workflow: `extract_text` over the relevant uploads, `pdf_to_images` for the relevant pages, and `view_image` on every page/crop needed to preserve source structure faithfully.
...
- Uploaded-work grading (from `/spark` chat or from a ready-to-solve draft sheet) is implemented as an Agent run:
  ...
  - Transcription/source gathering stays on the main agent. Generic `spawn_agent` is disabled for grader intake/file-reading/transcription so the model cannot delegate basic workspace access to a tool-poor context. Each final linked figure/image crop used in the worksheet requires its own `validate_crop_with_fresh_agent` validation pass with `view_image`, question context, reviewer-visible text transcription, and a pass/fail judgement; `grader/output/crop-validation.md` records each final linked crop path with `fresh-context subagent checked: yes`, the reviewer-visible text, and the pass/fail outcome. The main agent always consolidates final outputs.
  ...
  - If official references are allowed and the source is an identified public paper, the grader should use official answer keys, mark schemes, or official solutions for the correct-answer baseline before self-solving. If online lookup is disallowed or no official source is available, the grader solves each problem carefully itself; the derived solution should stay at the student's level and reuse their terminology/method style where reasonable.
  ...
  - After a grader run completes successfully, Spark automatically schedules a sheet-dashboard refresh agent. That agent receives the latest graded sheet plus the full graded-sheet dashboard snapshot and republishes `/spark/sheets` insights in the background.
  - Spark chat also exposes `refresh_sheet_dashboard`; use it when the learner asks for subject filters, strong spots, weak spots, or a focused dashboard refresh. The tool launches the same background dashboard agent with an optional focus note rather than answering from stale chat memory.
...
- `spark/{uid}/sheetDashboard/current`: the published `/spark/sheets` dashboard state. The document stores `{ schemaVersion: 1, mode: 'sheet_dashboard', headline, summaryMarkdown?, focusNote?, generatedFromRunId?, strengths: Array<{ id, title, summary, evidenceRunIds: string[], subjectKeys: string[], specifics: string[], nextSteps: string[], generalFeedback? }>, weakSpots: Array<...>, subjects: Array<{ key, label, summary, runIds: string[], averagePercentage?, strongSpots: string[], weakSpots: string[], specifics: string[], nextSteps: string[], generalFeedback? }>, runAnalyses: Array<{ runId, subjectTags: Array<{ key, label }>, primarySubjectKey?, summary?, strongSpots: string[], weakSpots: string[], specifics: string[], nextSteps: string[], generalFeedback? }>, updatedAt }`. `/spark/sheets` uses `runAnalyses` for subject filters/tag badges and the top-level arrays for the dashboard detail panel, with `specifics` / `nextSteps` rendered ahead of any general feedback.
