<script lang="ts">
  ...
  function resolveFirst(source: UserSnapshot, fallback = 'friend'): string {
    ...
    const base = name && name.length > 0 ? name : emailHandle && emailHandle.length > 0 ? emailHandle : fallback;
...
<section class="welcome-dashboard">
  <div class="welcome-panel">
    ...
    <p class="welcome-subtitle">Ignite your coding journey with a session tailored for curious first-timers.</p>
    ...
    <div class="welcome-cards">
      {#if options.length === 0}
        <p class="empty-copy">No starter sessions are available right now. Please try again soon.</p>
      {:else}
