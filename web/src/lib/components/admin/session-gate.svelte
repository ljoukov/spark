<script lang="ts">
	...
	const signedInUser = $derived(session.status === 'admin' || session.status === 'not_admin' ? session.user : null);
...
<div class="min-h-[100vh] bg-background px-4 py-16">
	<div class="mx-auto flex max-w-lg flex-col gap-10">
		...
		<section class="rounded-3xl border border-border bg-card p-8 shadow-xl shadow-black/5 backdrop-blur">
			{#if isAdmin}
				<div class="flex flex-col gap-4 text-sm">
					...
					<button
						type="button"
						class="inline-flex items-center justify-center rounded-full bg-destructive px-5 py-2 text-sm font-medium text-destructive-foreground transition hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-60"
			...
			{#if ui.successMessage}
				<p class="mt-6 rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-700">
					{ui.successMessage}
				</p>
			{/if}
		</section>
