...
const remainderBucketsProblem: CodeProblem = {
  ...
  solution: {
    optimal: {
      ...
      overview: 'Bucket each value by remainder and accumulate c * (c - 1) / 2 for each bucket size c.',
      steps: [
    ...
    alternatives: [
      {
        ...
        overview: 'Update the answer as you go: each new element in a bucket adds existing_count pairs.',
        steps: [
...
const xorLampGridProblem: CodeProblem = {
  ...
  solution: {
    optimal: {
      ...
      overview: 'Track lamps with odd toggle count in a set; add when toggled ON, remove when returning to OFF.',
      steps: [
      ...
      spaceComplexity: 'O(k)',
      keyIdeas: [
        'Sets capture XOR parity directly',
        'Sorting at the end keeps output tidy',
      ],
    },
    alternatives: [
      {
        title: 'Count array in place',
        overview: 'Maintain integer counts per lamp and check count % 2 at the end. Uses more memory when count is large but keeps output sorted.',
        steps: [
...
const nimBalanceProblem: CodeProblem = {
  ...
  solution: {
    optimal: {
      ...
      overview: 'Fold the array with XOR. Zero nim-sum means the current player is already at a disadvantage.',
      steps: [
...
const nineAdjusterProblem: CodeProblem = {
  ...
  constraints: [
    '1 <= len(value) <= 100000',
    'value consists only of decimal digits',
  ],
  edgeCases: [
  ...
  solution: {
    optimal: {
      ...
      timeComplexity: 'O(n)',
      spaceComplexity: 'O(1)',
      keyIdeas: ['Modulo 9 complement chooses the smallest fixer', 'String iteration avoids big integer issues'],
    },
    alternatives: [
      {
        ...
        timeComplexity: 'O(n)',
        spaceComplexity: 'O(1)',
        keyIdeas: ['Ties directly to digital root computation', 'Reuse previous helper if implemented'],
      },
    ],
  },
  source: {
...
const modularMagicQuizzes: QuizDefinition[] = [
  {
    ...
    questions: [
      ...
      },
      {
        ...
        explanation: 'It means 29 leaves remainder 1 when divided by 7 (29 - 1 is a multiple of 7).',
        options: [
    ...
    questions: [
      {
        kind: 'multiple-choice',
        id: 'xor-review-q1',
        prompt: 'Why can XOR find the single unique element in an array where every other element appears twice?',
        hint: 'Think cancellation.',
        explanation: 'Pairs cancel to 0 and only the unique element remains in the running XOR.',
        options: [
          { id: 'A', label: 'A', text: 'Because XOR cancels matching values and leaves the unique one' },
      ...
      },
      {
        ...
        explanation: 'A zero nim-sum means the next player loses with optimal play if the opponent plays perfectly.',
        options: [
...
const digitalRootsQuizzes: QuizDefinition[] = [
  {
    ...
    questions: [
      ...
      },
      {
        kind: 'multiple-choice',
        id: 'digital-intro-q2',
        prompt: 'If two numbers have the same digital root, what can you say about their difference?',
        hint: 'Think modulo 9.',
        explanation: 'They differ by a multiple of 9, because digital roots encode numbers modulo 9.',
        options: [
  ...
  },
  {
    ...
    questions: [
      ...
      },
      {
        ...
        prompt: 'You have the string "827". What single digit should you append so the digit sum becomes divisible by 9?',
...
export async function provisionWelcomeSession(
  ...
  await Promise.all(template.problems.map((problem) => saveUserProblem(userId, session.id, problem)));
