...
export async function getGoogleAccessToken(options: {
	...
	const { serviceAccountJson, scopes } = options;
	const sa = parseGoogleServiceAccountJson(serviceAccountJson);
	...
	if (!resp.ok) {
		...
		throw new Error(`Google OAuth token exchange failed (${resp.status}): ${text.slice(0, 500)}`);
	}
