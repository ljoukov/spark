...
function parseStages(argv: readonly string[]): Stage[] {
	const stageFlagIndex = argv.findIndex((arg) => arg === '--stage' || arg.startsWith('--stage='));
	if (stageFlagIndex === -1) {
		return [...ALL_STAGES];
	}
	...
	const stages = parts.map((part) => {
		try {
			...
			throw new Error('[audit-eval] Invalid stage value. Use "stats", "reports", or "all".');
		}
	});
...
function summariseEvaluations(evaluations: LoadedEvaluation[], logger: StatsLogger): void {
	...
	if (evaluationsWithAudit.length === 0) {
		logger.log('[analysis] No audit data available; skipping combined metrics.');
		return;
	}
	...
	printJudgementMetrics('Auditor flagged follow-up', flaggedEvaluations, logger);
...
function printJudgementMetrics(
	...
	logger.log(`[analysis] ${label}: ${total} evaluation${total === 1 ? '' : 's'}.`);
	if (total === 0) {
		logger.log('  - None');
		return;
	}
...
function printHighConfidenceBreakdown(evaluations: LoadedEvaluation[], logger: StatsLogger): void {
	logger.log(
	...
	for (const [criterion, data] of criteria) {
		...
		for (const [bucket, count] of buckets) {
			...
			logger.log(`    - ${bucket}: ${formatPercentage(percentage)} (${count}/${data.total})`);
		}
	}
}
...
function collectFailureCases(evaluations: readonly LoadedEvaluation[]): Map<string, FailureCase[]> {
...
async function runStats(loadResult: LoadResult): Promise<void> {
	...
	try {
		...
		errorLine(`[analysis] ERROR failed to write stats to ${STATS_OUTPUT_PATH}: ${message}`);
		if (!summariseError) {
...
async function runReports(evaluations: readonly LoadedEvaluation[]): Promise<void> {
