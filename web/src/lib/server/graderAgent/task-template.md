...
6. For handwritten papers >12 leaves or >30 marks, call `score_answers_with_fresh_agent` after the minimal plan in 2-4 contiguous root/range batches. Pass `studentImagePaths` for relevant original answer pages whenever marks depend on ticks, circles, shading, graphs, diagrams, calculation layout, or uncertain handwriting. Use returned question/modelAnswer results directly, preserving `teacher-review`. Do not hand-score. Do not reread every scoring file.
