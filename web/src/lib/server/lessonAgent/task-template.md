...
- `lesson/output/code/<planItemId>.json` — CodeProblem docs, one per plan item with `kind="coding_problem"`
...
- `lesson/schema/coding_problem.schema.json` — JSON schema for `lesson/output/code/*.json`
...
1) Read `brief.md` + `request.json` + this file, then write hard requirements + key decisions to `lesson/requirements.md`.
   - If the request mentions **6-8 marker** (or similar), record it as a hard requirement and apply it to the quiz `type-answer` question(s): `marks` must be in **[6,8]** with a clear bullet `markScheme` allocating those marks.
2) Decide lesson shape:
   - If this is programming practice, include `kind="coding_problem"` steps; otherwise make it quiz-only (no `coding_problem` steps).
...
4) Ordering:
   - If coding is included: draft/verify coding problems first, then draft the session plan/quizzes from the verified coding problems.
...
5) Compile Firestore-ready JSON outputs under `lesson/output/` from the Markdown drafts:
   ...
   - `lesson/output/code/<planItemId>.json` — for each `plan[]` item with `kind="coding_problem"`
...
6) Validate + publish:
   - Call `publish_lesson` with:
     ...
     - `includeCoding: true|false` (based on whether you included `coding_problem` steps)
...
- Use LaTeX for maths/science formulas in quiz/coding problem Markdown (`$...$` inline, `$$...$$` display). Do not wrap `$...$` / `$$...$$` formulas in backticks (inline code), or they will render as `<code>` instead of math.
