...
- GCP production uses Cloud Run service `spark` in region `us-central1`, with Cloud Tasks queue `spark-tasks` in the same region. The internal task handler is `POST /api/internal/tasks`.
- The GitHub-triggered Cloud Build pipeline is regional, not global. The `spark` trigger lives in `us-central1` and points at `web/cloudbuild.yaml`; when inspecting builds or triggers with `gcloud`, include `--region us-central1` or the trigger/build list may appear empty.
- Secret Manager secret `env-prod` is the canonical hosted runtime env bundle. `web/write_dotenv.py` derives two files from it during Cloud Build: `web/.env.local` for the image build and `web/cloud-run-env.yaml` for `gcloud run deploy --env-vars-file`.
- Hosted runtime env must come from the Cloud Run service configuration, not from a copied `.env.local` inside the image. The runtime image should not embed secret env files.
- The Cloud Build deploy step replaces Cloud Run env with `--env-vars-file`; this is the mechanism that prevents stale service-level env drift (for example leftover API-key vars) from surviving later deploys.
- For hosted task smoke tests, prefer the real queue path over direct handler calls: point `TASKS_SERVICE_URL` at `https://spark-1083072308192.us-central1.run.app`, enqueue `createTask({ type: "helloWorld" })`, then confirm both the Cloud Run request log and stdout line `[internal task] helloWorld`.
- Recent Cloud Run revisions are commonly deployed by two identities depending on path: the regional trigger/manual Cloud Build path uses `server@pic2toon.iam.gserviceaccount.com`, while ad-hoc `gcloud run services update` operations may appear under the active operator identity. Check the revision list before assuming which path last modified production.
- Vercel production is separate from GCP/Cloud Run. The Vercel project is `ljoukovs-projects/spark`, the public production domain is `https://spark.eviworld.com`, and the app root on Vercel is `web/`.
- For Vercel production debugging, validate the real user path: login via `/login-with-email`, open `/spark`, send a prompt, and confirm the assistant response is a normal reply rather than an error state. Use Vercel CLI/logs plus browser automation when checking regressions.
