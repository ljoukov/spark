...
function resolveStoryCheckpointDir(sessionRootDir: string): string {
...
async function ensureStoryCheckpointDir(
  ...
  const targetDir = resolveStoryCheckpointDir(sessionRootDir);
  ...
  const migrateDirContents = async (
    ...
    for (const entry of entries) {
      ...
      if (await pathExists(toPath)) {
        console.warn(
          `[test-session/${sessionId}] checkpoint ${entry} already exists in ${targetDir}; leaving ${fromPath} untouched`,
        );
        continue;
      }
    ...
    console.log(
      `[test-session/${sessionId}] migrated checkpoints from ${sourceLabel} to ${targetDir}`,
    );
    return true;
  };
