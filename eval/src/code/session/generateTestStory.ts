...
async function generateStoryImages(
  ...
  const elapsed = formatMillis(Date.now() - startTime);
  progress.log(
    `[story/images] model ${modelVersion} generated ${formatInteger(imageCount)} illustrations in ${elapsed}`
  );
}
...
async function generateStorySegmentation(
  ...
  const elapsed = formatMillis(Date.now() - startTime);
  progress.log(`[story/segments] model ${modelVersion} finished in ${elapsed}`);
  ...
  await runJobsWithConcurrency({
    ...
    handler: async (options, { progress }) => {
      ...
      if (shouldGenerateImages) {
        ...
        await generateStoryImages(segmentation, outDir, progress);
      }
