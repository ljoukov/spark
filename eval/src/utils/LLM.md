...
- Prompt inline images are converted to JPEG when possible and stored under `media/{sha256}.jpg`. The short label shown in `prompt.txt` (e.g. `prompt-image-001-abc123`) matches the first six characters of the SHA and helps correlate parts with files.
- For image calls, output images are handled the same way: JPEG-converted buffers are written to `media/{sha256}.jpg`, and the short label recorded in `response.txt` (e.g. `image-001-def456`) references the same hash.
