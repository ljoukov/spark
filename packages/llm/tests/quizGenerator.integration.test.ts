...
describe.sequential("Gemini quiz generation pipeline", () => {
  ...
  test(
    ...
    async () => {
      ...
      const typeSet = new Set(
        synthesisQuiz.questions.map(
          (question: QuizGeneration["questions"][number]) => question.type,
        ),
      );
  ...
  test(
    ...
    async () => {
      ...
      const basePrompts = new Set(
        synthesisQuiz.questions.map(
          (question: QuizGeneration["questions"][number]) =>
  ...
  test(
    "judge evaluates generated quiz against rubric",
    { timeout: LONG_TIMEOUT },
    async () => {
      await ensureSources();
      if (!extractionQuiz) {
  ...
  test(
    ...
    async () => {
      ...
      expect(["agree", "needs_review", "disagree"]).toContain(
        audit.verdictAgreement,
      );
