...
function convertTypeAnswerQuestion(
  ...
  return {
    ...
    hint: question.hint?.trim(),
    correctFeedback: {
      message: "Nice work — you covered the key points.",
      tone: "success",
    },
...
function buildSparkUploadQuizPrompt(
  ...
  const lines: string[] = [
    ...
    "- Do not mention the source documents, sections, or page numbers in the questions or multiple-choice explanations.",
    "- Provide hints sparingly; they should nudge, not give away the answer.",
    "- Multiple-choice explanations must justify the correct response referencing the source.",
    ...
    "- Type-answer: keep answers short; include acceptableAnswers when multiple phrasings are equivalent; include marks (3-6 preferred) and a concise markScheme. Do not include an explanation field for type-answer questions.",
