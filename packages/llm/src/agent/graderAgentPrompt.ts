...
export function buildSparkGraderAgentPrompt(options?: {
  ...
  return [
    ...
    "- Use bounded subagents only for official-reference lookup/verification when online lookup is allowed, or for visual localization proposals that keep image inspection out of the main context.",
    ...
    "",
    "Deliverables:",
    "1) Write `grader/output/transcription.md` from a transcription-first extraction pass, then normalize student work into numbered statements/sentences rather than a summary. When grading against a source paper, include a compact `## Source problem-statement transcription` audit section even when source text came from deterministic PDF extraction.",
    ...
    `3) Write ${summaryPath} with this exact top-level shape: \`{ "totals": { "awardedMarks": number, "maxMarks": number }, "presentation": { "title": string, "subtitle": string, "summaryMarkdown": string, "footer": string }, "sheet": { "title": string, "filePath": "${sheetPath}" } }\`, plus optional paper metadata only when known; if you include \`year\`, write it as a string such as \`"2024"\``,
    ...
    "8) Call publish_sheet({}) to validate and publish the worksheet artifact; if it fails, use the validation error as a diagnostic, repair the artifacts coherently, and retry only while you are fixing a new class of issue rather than repeating the same failed branch",
    "9) Do not send a normal final response before publishing. If you catch yourself about to answer the user, instead keep using tools, write the required files, call publish_sheet, then call done.",
  ].join("\n");
}
