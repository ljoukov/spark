...
export function renderSparkGraderTask(taskTemplate: string): string {
  ...
  const graderOverrides = [
    ...
    "- Use `extract_pdf_images` as a deterministic embedded-raster first pass when useful, `propose_crop_bbox_with_fresh_agent` for uncertain crop bbox planning, `validate_crop_with_fresh_agent` for final crop validation, and `review_run_progress_with_fresh_agent` when repeated loops suggest the run is on the wrong path.",
    "- For crop validation, pass one final crop path plus question context to `validate_crop_with_fresh_agent`; include `expectedContent` with the exact visual labels/axes/options/table cells needed in the crop and `duplicatedTextToExclude` for surrounding prompt/caption/table text rendered separately. Do not batch several high-risk crops into one review.",
    ...
    "- Final grading feedback must be emitted as per-question worksheet scores plus worksheet-level reference markdown. Correct/resolved questions may use an empty `note` because completed feedback cards are hidden in the student UI.",
