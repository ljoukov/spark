...
function inferSourcePaperOnlyNoStudentRequest(options: {
  ...
  const requestText = [options.sourceText, options.input.title, options.input.notes]
...
export function renderSparkGraderTask(taskTemplate: string): string {
  ...
  const transcriptionSkillSection = [
    ...
    "- When a figure/photo/diagram matters, crop it into `grader/output/assets/...` and reference it from worksheet Markdown with a workspace-relative clickable image link such as `[![Figure 1](grader/output/assets/q1-figure-1.png)](grader/output/assets/q1-figure-1.png)`.",
